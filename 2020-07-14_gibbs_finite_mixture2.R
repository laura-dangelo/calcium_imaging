...
data <- sim_data(n = 500, lambda = 10, time_spike = c(50,52, 140, 180,250,350,420,460),
                 gamma = 0.8, b = 0,
...
gibbs_calcium <- function(nrep, y,
                          ...
                          p = 0.995, 
                          # Dirichlet parameter, variance of G0, mean Poisson
                          alpha = 1, psi2 = 2, pois_mean = 2, trunc = 300,
                          # hyperparameters
                          hyp_A1 = 3, hyp_A2 = 1, hyp_gamma1 = 5, hyp_gamma2 = 2,
                          hyp_lambda1 = 10, hyp_lambda2 = 1, hyp_b1 = 1, hyp_b2 = 1,
                          # MH step
                          eps_gamma)
{
  ...
  for(i in 1:(nrep-1))
  {
    ...
    for(j in 1:n)
    {
      ...
      if(length(nj)>0)
      {
        pr_clus = (nj + alpha) * (1-p) * sapply(sort(unique(clus_tmp[-j][clus_tmp[-j]>0])),
                                                function(x) dnorm(y[j], mean = out_b[i+1] + out_gamma[i+1] * out_c[i+1,j] + out_A[i+1,x+1],
                                                                  sd = sqrt(sigma2 + tau2)) ) 
      }
      
      v_ratio = v(n = n-1, t = length(nj)+2, alpha = alpha, pois_mean = pois_mean, trunc = trunc) /
        v(n = n-1, t = length(nj)+1, alpha = alpha, pois_mean = pois_mean, trunc = trunc)
      if(is.nan(v_ratio)) v_ratio = 0
      
      pr_new = (1-p) * v_ratio * alpha * marg(y = y[j], b = out_b[i+1], c = out_c[i+1,j], gamma = out_gamma[i+1],
...
#c(50,52, 140, 180,250,350,420,460)


which(apply(prova$A, 2, function(x) sum(!is.na(x))>0))
prova$A = prova$A[,1:14]
...
prova$A[500:540,]
    ...
    for(j in 1:n)
    {
      ...
      v_ratio = v(n = sum(clus_tmp[-j]>0), t = length(nj)+1, alpha = alpha, pois_mean = pois_mean, trunc = trunc) / 
          v(n = sum(clus_tmp[-j]>0), t = length(nj), alpha = alpha, pois_mean = pois_mean, trunc = trunc)
      ...
      if(length(nj)>0)
      {
        pr_clus = sapply(sort(unique(clus_tmp[-j][clus_tmp[-j]>0])), 
                         function(x) dnorm(y[j], mean = out_b[i+1] + out_gamma[i+1] * out_c[i+1,j] + out_A[i+1,x+1], 
                                           sd = sqrt(sigma2 + tau2)) ) * (nj + alpha) * (1-p)
      }
      clus_tmp[j] = sample( -1:length(pr_clus), 1, prob = c(pr_new, pr0, pr_clus) )
