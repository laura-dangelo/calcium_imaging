                               ...
                               double eps_gamma)
{
  ...
  for(int i = 0; i < Nrep - 1; i++)
  {
    ...
    for(int j = 0; j < n; j++)
    {
      cluster(j, i+1) = (-99) ; // remove the j-th element
      
      arma::vec line(n); line = cluster.col(i+1) ;
      arma::vec non0 = line.elem( find( line > 0 ) );
      arma::vec tmp = arma::unique( non0 ) ; // unique labels
      double max_clus = non0.max() ; // max label of the cluster
      
      int n_clus = tmp.n_elem ; // number of clusters 
   ...
   //     Rcout << "max " << max_clus << "\n" ;
    //    Rcout << "n-clus " << n_clus << "\n" ;
        ...
        {
          arma::uvec ids = find( line == k ) ; // qui è sbagliato!
          
          arma::uvec ind = i * cluster.n_rows + ids ;
          arma::vec el = cluster.elem(ind) ;
      //    Rcout << "elem " << el << "\n" ;
          
          cluster.elem(ind).fill(k - 1) ;
          out_A(k-1, i+1) = out_A(k, i+1) ;
        }
        out_A(max_clus, i+1) = 0 ;
      }
      
      double pr0 = p * R::dnorm(y(j), out_b(i+1) + out_gamma(i+1) * out_c(j, i+1), std::sqrt(sigma2 + tau2), false) ;
      double pr_new = (1-p) * alpha * marginal(y(j), out_c(j, i+1), out_b(i+1), out_gamma(i+1), sigma2 + tau2, psi2) ;
      ...
      if(n_clus > 0)
      {
        ...
        for(int k = 1; k < n_clus + 1; k++)
        {
          ...
          prob(k) = nj(k-1) * (1-p) * R::dnorm(y(j), out_b(i+1) + out_gamma(i+1) * out_c(j, i+1) + out_A(k, i+1), std::sqrt(sigma2 + tau2), false) ;
        }
      }
      ...
      cluster(j, i+1) = RcppArmadillo::sample( clusters_id, 1, false, prob )[0] ;
      
      if(cluster(j, i+1) == n_clus + 1)
      {
        out_A(n_clus + 1, i+1) = gen_truncnorm(psi2 / (psi2 + sigma2 + tau2) * (y(j) - out_b(i+1) - out_gamma(i+1) * out_c(j, i+1)), 
