...
# save(run4, file = "res_sim_1709_par4_low.Rdata")

burnin = 1
plot(1:length(run4$p[-burnin]), run4$p[-burnin], type = "l")
lines(1:length(run4$p[-burnin]), cumsum(run4$p[-burnin])/1:length(run4$p[-burnin]), col =2)

plot(1:length(run4$sigma2[-burnin]), run4$sigma2[-burnin], type = "l", xlab = "iterazioni", ylab = "sigma2")
lines(1:length(run4$sigma2[-burnin]), cumsum(run4$sigma2[-burnin])/1:length(run4$sigma2[-burnin]), col =2)

plot(1:length(run4$tau[-burnin]), run4$tau[-burnin], type = "l", xlab = "iterazioni", ylab = "tau2")
lines(1:length(run4$tau[-burnin]), cumsum(run4$tau[-burnin])/1:length(run4$tau[-burnin]), col =2)

plot(1:length(run4$b[-burnin]), run4$b[-burnin], type = "l", xlab = "iterazioni", ylab = "b")
lines(1:length(run4$b[-burnin]), cumsum(run4$b[-burnin])/1:length(run4$b[-burnin]), col =2)

plot(1:length(run4$gamma[-burnin]), run4$gamma[-burnin], type = "l", xlab = "iterazioni", ylab = "gamma")
lines(1:length(run4$gamma[-burnin]), cumsum(run4$gamma[-burnin])/1:length(run4$gamma[-burnin]), col =2)
...
AA = matrix(0,(length(run2$b)-max(burnin)),n)
for(i in 1:(length(run2$b)-max(burnin)) )
{
  ii = i + (max(burnin))
  AA[i, t(run2$clus)[ii,] >0] = run2$A[run2$clus[run2$clus[,ii] >0,ii]+1,ii]
}

est_spikes = colMeans(AA)
est_spikes[which( apply(t(run2$clus)[-burnin,], 2, function(x) mean(x != 0))<0.8)] = 0

int = 1:n
plot(int, y2[int], type = "l")
lines(int, rowMeans(run2$calcium[,-burnin])[-1][int], col="turquoise", lwd = 1.2)
...
ggplot(data = data.frame(x = rep(int,2), 
                         y = c( y2[int], rowMeans(run2$calcium[,-burnin])[-1][int] ), 
...
{
  ii = i 
  AA[i, t(run4$clus)[ii,] >0] = run4$A[run4$clus[run4$clus[,ii] >0,ii]+1,ii]
}

est_spikes1 = colMeans(AA) ## run4
est_spikes1[which( apply(t(run4$clus)[-burnin,], 2, function(x) mean(x != 0))<0.8)] = 0
...
{
  ii = i 
  AA[i, t(run6$clus)[ii,] >0] = run6$A[run6$clus[run6$clus[,ii] >0,ii]+1,ii]
}

est_spikes2 = colMeans(AA) ## run6
est_spikes2[which( apply(t(run6$clus)[-burnin,], 2, function(x) mean(x != 0))<0.9)] = 0
...
int = 4600:4800
ggplot(data = data.frame(x = rep(int,2), 
                         y = c( y2[int], rowMeans(run6$calcium[,-burnin])[-1][int] ), 
                         col = as.factor( c(rep(1, length(int)), rep(2, length(int)) ))), 
       aes(x = x, y = y ) ) + 
  geom_line(aes(color = col)) +
  theme_bw() +
...
### number of clusters
plot(1:(nrep-max(burnin)), apply(run2$cluster[,-burnin], 2, function(x) length(unique(x[x>0]))), pch=19, cex=0.2)
table(apply(run2$cluster[,-burnin], 2, function(x) length(unique(x[x>0]))))/1500
