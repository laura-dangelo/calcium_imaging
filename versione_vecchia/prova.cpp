               ...
               double hyp_gamma1, double hyp_gamma2)
{
  ...
  double out = arma::accu(llik) + R::dbeta( gamma, hyp_gamma1, hyp_gamma2, true);
  return(out) ;
}
                   ...
                   double sigma2, double psi2)
{
  ...
  out = 2 + R::dnorm(y, mean1, sd1, true) + std::log( R::pnorm(0, mean2, sd2, false, false) );
  return(out);
}
...
arma::vec polya_urn(const arma::vec& y, arma::vec cluster, const arma::vec& cc,
                    const arma::vec& A, double b, double gamma, 
                    double p,
  ...
  for(int j = 0; j < n; j++)
  {
    ...
    if(n_clus > 0)
    {
      for(int k = 1; k < n_clus + 1; k++)
      {
        ...
        prob[k] = exp( log(nj) + log(1-p) + R::dnorm(y(j), b + gamma * cc(j) + A_tmp(k), std::sqrt(sigma2 + tau2), true) ) ;
      }
    }
    ...
    //arma::vec pp(n_clus + 2) ; 
    //pp =  prob/c_norm; 
    //cluster(j) = RcppArmadillo::sample( clusters_id, 1, false, pp )(0) ;
    
    if(cluster(j) == (n_clus + 1) )
    {
      A_tmp(n_clus + 1) = gen_truncnorm(psi2 / (psi2 + sigma2 + tau2) * (y(j) - b - gamma * cc(j)), 
                               ...
                               double eps_gamma)
{
  ...
  for(int i = 0; i < Nrep - 1; i++)
  {
    ...
    for(int j = 0; j < n; j++) { AA(j) =  out_A(cluster(j,i), i) ; }
    ...
    out_c(n, i+1) = R::rnorm(filter_mean(n-1), filter_var(n-1)) ;
    
    for(int j = n-2; j > -1; j--)
    {
      back_mean = filter_mean(j) + out_gamma(i) * filter_var(j) / R(j+1) * (out_c(j+2, i+1) - a(j+1)) ;
      back_var = filter_var(j) - pow(out_gamma(i) * filter_var(j), 2) / R(j+1) ;
      
      out_c(j+1, i+1) = R::rnorm(back_mean, back_var) ;
    }
    back_mean = out_gamma(i) * varC0 / (varC0 + tau2) * out_c(1, i+1) ;
    back_var = varC0 - pow(out_gamma(i) * varC0, 2) / R(0) ;
    out_c(0, i+1) = R::rnorm(back_mean, back_var) ;
    
    
    // sampling lambda (precision)
    arma::vec z(n) ; arma::vec sq(n) ; 
    for(int j = 0; j < n; j++) { z(j) = y(j) - out_gamma(i) * out_c(j, i+1) - AA(j) ; }
    ...
    double num = logpost(y, out_c.col(i+1), 
                         AA, out_b(i+1), 
                         newgamma, out_lambda(i+1),
                         hyp_gamma1, hyp_gamma2) ;
    double den = logpost(y, out_c.col(i+1), 
                         AA, out_b(i+1), 
    ...
    arma::vec non0A = line.elem( find( line > 0 ) );
    arma::vec tmpA = arma::unique( non0A ) ; // unique labels
    
    double n_clusA = tmpA.n_elem ; // number of clusters (n.unique \{0}) ...
    arma::vec njA(n_clusA) ;
    arma::vec ssum(n_clusA) ;
    
    arma::vec lincomb(n) ;
    for(int l = 0; l < n; l++) { lincomb(l) = y(l) - out_b(i+1) - out_gamma(i+1) * out_c(l, i+1) ; }
    
    for(int k = 1; k < n_clusA + 1; k++) 
    { 
      njA(k-1) = std::count(non0A.begin(), non0A.end(), k) ; 
      arma::uvec idk = find( line == k ) ;
      ssum(k-1) = arma::accu( lincomb(idk) ) ;
      
      out_A(k, i+1) = gen_truncnorm( psi2 * ssum(k-1) / (njA(k-1) * psi2 + sigma2 + tau2), 
            std::sqrt((sigma2 + tau2) * psi2 / (njA(k-1) * psi2 + sigma2 + tau2) )) ;
    }
    if(check == 1) 
    {  
      Rcout << "Allarme allarme!! " << i << "\n" ; 
      i = i-100;
    }
    if(out_b(i+1) < -5) {Rcout << "Allarme b!! " << i << "\n";
      i = i-50 ;}
