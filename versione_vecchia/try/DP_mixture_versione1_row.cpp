                               ...
                               double eps_gamma)
{
  ...
  for(int i = 0; i < Nrep - 1; i++)
  {
    ...
    for(int j = 0; j < n; j++)
    {
      cluster(i+1, j) = (-99) ; // remove the j-th element
      
      arma::rowvec line(n); line = cluster.row(i+1) ;
      ...
      if(max_clus > n_clus) // it means that there is a "hole" in the label sequence
      {
       ...
       for(int k = diff + 1 ; k < max_clus +1; k++)
        {
          arma::uvec ids = find( non0 == k ) ; // qui è sbagliato!
          arma::uvec ind = ((ids - 1) * cluster.n_rows + i) ;
...
//        A_tmp(max_clus) = 0 ;
      ...
      cluster(i+1, j) = RcppArmadillo::sample( clusters_id, 1, false, prob )[0] ;
      
      if(cluster(i+1, j) == n_clus + 1)
      {
