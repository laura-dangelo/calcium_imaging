...
gibbs_calcium <- function(nrep, y,
                          ...
                          # (fixed) variance of the underlying calcium
                          tau2 = 0.0001,
                          ...
                          eps_gamma)
{
  ...
  out_A = matrix(0, nrep, 100) # contiene i valori A1,...,Ak
  out_b = rep(NA, nrep)
  ...
  for(i in 1:(nrep-1))
  {
    ...
    cluster[i+1,] = polya_urn(y = y, cluster = cluster[i,], cc = out_c[i+1,], A = A_tmp, b = out_b[i+1],
                   gamma = out_gamma[i+1], p = p, sigma2 = sigma2, tau2 = tau2, alpha = alpha, psi2 = psi2 )
...
data <- sim_data(n = 1000, lambda = 8, time_spike = c(100, 101, 150, 300, 400,402, 500, 700, 720, 850),
                 gamma = 0.8, b = 2,
                 prob = c(0.1, 0.7, 0.2), par = c(3, 10, 6))
...
which(apply(prova$A, 2, function(x) sum(!is.na(x))>(nrep/10)))
prova$A = prova$A[,1:10]
prova$A[is.na(prova$A)] = 0

maxA = apply(prova$A[-burnin,], 1, max)
mean(maxA)
...
which(apply(prova$clus[-burnin,], 2, function(x) sum(x==0) ) == 0)
