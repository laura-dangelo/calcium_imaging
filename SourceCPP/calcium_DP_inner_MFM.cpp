...
/*
 * A ~ G0 = Gamma(hyp_A1, hyp_A2) media è hyp_A1/hyp_A2
...
double log_den(double alpha, int k, int n)
{
  double out = 0 ;
  arma::vec tmp(n) ;
  for(int l = 0; l < n ; l++ )
...
// [[Rcpp::export]]
double logV_n_t(double alpha, double pois_par, int n, int t, int u_bound)
{
  arma::vec tmp(u_bound) ;
  tmp.fill(0) ;
  for(int k = 0; k < u_bound ; k++)
  {
    tmp(k) = exp( log_num(k + 1, t) - log_den(alpha, k + 1, n) + R::dpois(k + 1, pois_par, true) );
  }
                      ...
                      const arma::vec& log_V)
{
  ...
  for(int j = 0; j < n; j++)
  {
    ...
    if(non0.n_elem > 0) 
    {
      arma::vec unique_non0 = arma::unique( non0 ) ; // unique labels of existing clusters (-j)
      ...
      if(max_clus > n_clus) 
      {
        ...
        for(int l = 0; l < m-1; l++) { A_tmp(max_clus + 1 + l) = sample_mix(p, hyp_A1, hyp_A2) ; }
      }
    }
    // end if( non0.elem>0 )
    ...
    if(max_clus == n_clus)
    {
      ...
      for(int l = 0; l < m; l++) { A_tmp(max_clus + 1 + l) = sample_mix(p, hyp_A1, hyp_A2) ; }
    }
    ...
    if(n_clus > 0)
    {
      ...
      {
        int nj = std::count(non0.begin(), non0.end(), k) ;
        n0 = n0 - nj ;
        prob[k] = log(nj + alpha) + R::dnorm(y(j), b + gamma * cc(j) + A_tmp(k), std::sqrt(sigma2 + tau2), true) ;
      }
    }
    
    prob[0] = log(n0 + alpha) + R::dnorm(y(j), b + gamma * cc(j), std::sqrt(sigma2 + tau2), true) ;
    
    double logcoef =  log_V(n_clus + 1) - log_V(n_clus) ; 
    
    for(int k = n_clus + 1; k < n_clus + m + 1; k++)
    {
      prob[k] = logcoef + log(alpha) - log(m) + R::dnorm(y(j), b + gamma * cc(j) + A_tmp(k), std::sqrt(sigma2 + tau2), true) ;
    }
    ...
    if( A_tmp(cluster(j)) == 0 ) { cluster(j) = 0 ; }
...
Rcpp::List calcium_gibbs(int Nrep, arma::vec y,
                         ...
                         int m, // number of temporary parameters polya urn
                         ...
                         double pois_par, int u_bound) 
{
  ...
  for(int t = 1; t < 51; t++)
  {
    log_V(t-1) = logV_n_t(alpha, pois_par, n, t, u_bound) ;
  }
  ...
  for(int i = 0; i < Nrep -1 ; i++)
  {
    ...
    //    if( !out_sigma2.is_finite() ) { check = 1 ; }
