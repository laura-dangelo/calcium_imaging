  ...
  for(int j = 0; j < n; j++)
  {
    ...
    if(non0.n_elem > 0) 
    {
      ...
      if(max_clus > n_clus) 
      {
        ...
        for(int l = 0; l < m-1; l++) { A_tmp(max_clus + 1 + l) = R::rgamma(hyp_A1, 1/hyp_A2) ; }
      }
    }
    // end if( non0.elem>0 )
    ...
    if(max_clus == n_clus)
    {
      ...
      for(int l = 0; l < m; l++) { A_tmp(max_clus + 1 + l) = R::rgamma(hyp_A1, 1/hyp_A2) ; }
    }
    ...
    {
      prob[k] = log(1-p) + log(alpha) - log(m)  - log(non0.n_elem - 1 + alpha) + R::dnorm(y(j), b + gamma * cc(j) + A_tmp(k), std::sqrt(sigma2 + tau2), true) ;
    }
