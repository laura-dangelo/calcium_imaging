  ...
  for(int j = 0; j < n; j++)
  {
    ...
    if(n_clus > 0)
    {
      for(int k = 1; k < n_clus + 1; k++)
      {
        ...
        n0 = n0 - nj ;
        prob[k] = log(nj) - log(n - 1 + alpha) + R::dnorm(y(j), b + gamma * cc(j) + A_tmp(k), std::sqrt(sigma2 + tau2), true) ;
      }
    }
    
    prob[0] = log(n0) - log(n - 1 + alpha) + R::dnorm(y(j), b + gamma * cc(j), std::sqrt(sigma2 + tau2), true) ;
    
    for(int k = n_clus + 1; k < n_clus + m + 1; k++)
    {
      prob[k] = log(alpha) - log(m)  - log(n - 1 + alpha) + R::dnorm(y(j), b + gamma * cc(j) + A_tmp(k), std::sqrt(sigma2 + tau2), true) ;
    }
